export default {
  async fetch(req, env) {
    ...
    // ===== ultra-safe schema (single-line SQL, no comments) =====
    async function ensureSchema() {
      const USERS_SQL = "CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY AUTOINCREMENT, phone TEXT UNIQUE NOT NULL, full_name TEXT, id_number TEXT, account TEXT UNIQUE, wallet INTEGER DEFAULT 0, invested INTEGER DEFAULT 0, pass_hash TEXT, created_at INTEGER)";
      const ADMINS_SQL = "CREATE TABLE IF NOT EXISTS admins (phone TEXT PRIMARY KEY, pass_hash TEXT NOT NULL, created_at INTEGER)";
      const TX_SQL = "CREATE TABLE IF NOT EXISTS transactions (id TEXT PRIMARY KEY, phone TEXT, type TEXT, amount INTEGER, detail TEXT, status TEXT, ts INTEGER)";
      const TX_IDX_SQL = "CREATE INDEX IF NOT EXISTS idx_tx_phone_ts ON transactions(phone, ts)";
    ...
    if (url.pathname === "/api/admin/init") {
      ...
      await q("INSERT INTO admins (phone, pass_hash, created_at) VALUES (?, ?, ?) ON CONFLICT(phone) DO UPDATE SET pass_hash=excluded.pass_hash",
        phone, hash, nowSec()).run();

      // echo columns to prove table exists
      const cols = await env.DB.prepare("PRAGMA table_info(users)").all();
      return json({ ok:true, adminPhone: phone, usersColumns: (cols.results||[]).map(c=>c.name) });
    }

    // -------- REGISTER (auto account) --------
    if (url.pathname === "/api/register" && req.method === "POST") {
      await ensureSchema();
      const { full_name, id_number, phone, pin } = await req.json().catch(()=>({}));
      if (!/^0(7|1)\d{8}$/.test(String(phone||"").trim())) return json({ ok:false, error:"Invalid phone" }, { status:400 });
      if (!/^\d{4}$/.test(String(pin||""))) return json({ ok:false, error:"PIN must be 4 digits" }, { status:400 });
      ...
      await q("INSERT INTO users (phone, full_name, id_number, account, wallet, invested, pass_hash, created_at) VALUES (?,?,?,?,0,0,?,?)",
    ...
    if (url.pathname === "/api/login" && req.method === "POST") {
      ...
      const pass_hash = await sha256Hex(`${pin}:${env.PASS_SALT || "imarika-salt"}`);
      const u = await q("SELECT phone, full_name, id_number, account, wallet, invested, created_at FROM users WHERE phone=? AND pass_hash=?", phone, pass_hash).first();
    ...
    if (url.pathname === "/api/tx" && req.method === "POST") {
      ...
      if (!phone || !type || !Number.isFinite(Number(amount))) return json({ ok:false, error:"phone/type/amount required" }, { status:400 });
      const id = crypto.randomUUID();
      await q("INSERT INTO transactions (id, phone, type, amount, detail, status, ts) VALUES (?,?,?,?,?,'Pending',?)",
    ...
    const needAdmin = async () => (await verifyAdmin()) ? true : false;
    ...
    if (url.pathname === "/api/admin/users" && req.method === "GET") {
      ...
      const res = await q(
        "SELECT full_name, phone, id_number, account, wallet, invested, created_at FROM users WHERE (?='' OR full_name LIKE ? OR phone LIKE ? OR account LIKE ?) ORDER BY created_at DESC LIMIT 500",
    ...
    if (url.pathname === "/api/admin/user" && req.method === "GET") {
      ...
      const u = await q("SELECT full_name, phone, id_number, account, wallet, invested, created_at FROM users WHERE "+where, val).first();
    ...
    if (url.pathname === "/api/admin/user/upsert" && req.method === "POST") {
      if (!(await needAdmin())) return json({ ok:false, error:"Unauthorized" }, { status:401 });
      await ensureSchema();
      const { phone, full_name=null, id_number=null, wallet=null, invested=null, account=null } = await req.json().catch(()=>({}));
      if (!phone) return json({ ok:false, error:"phone is required" }, { status:400 });

      const exists = await q("SELECT id FROM users WHERE phone=?", phone).first();
      if (exists) {
        await q("UPDATE users SET full_name=COALESCE(?,full_name), id_number=COALESCE(?,id_number), wallet=COALESCE(?,wallet), invested=COALESCE(?,invested), account=COALESCE(?,account) WHERE phone=?",
          full_name, id_number, wallet, invested, account, phone).run();
      } else {
        const acct = (account && String(account).trim()) || acctNum();
        await q("INSERT INTO users (phone, full_name, id_number, account, wallet, invested, created_at) VALUES (?,?,?,?,?,?,?)",
    ...
    if (url.pathname === "/api/admin/pending" && req.method === "GET") {
      if (!(await needAdmin())) return json({ ok:false, error:"Unauthorized" }, { status:401 });
      await ensureSchema();
      const res = await q("SELECT id, phone, type, amount, detail, status, ts FROM transactions WHERE status='Pending' ORDER BY ts DESC LIMIT 500").all();
