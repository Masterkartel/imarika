...
export const onRequest = async ({ request, env }) => {
  ...
  // ----- schema (compact SQL; no multiline parsing quirks) -----
  async function ensureSchema() {
    ...
    // run with batch so D1 parses each statement exactly as-is
    await env.DB.batch([
  ...
  const issueAdminToken = async () => {
    ...
    const payload = { sub: "admin", iat: nowSec(), exp: nowSec() + 86400 };
    const h = enc(header), p = enc(payload);
    const s = await signH(`${h}.${p}`, env.ADMIN_JWT_SECRET || "imarika-admin-secret");
    return `${h}.${p}.${s}`;
  };
  const verifyAdmin = async () => {
    const tok = (request.headers.get("authorization") || "").split(" ")[1] || "";
  ...
  // ----- health -----
  if (pathname === "/health") {
    return json({ ok: true, runtime: "pages-func-v3", time: Date.now() });
  }
  ...
  if (pathname === "/register" && request.method === "POST") {
    await ensureSchema();
    const { full_name, id_number, phone, pin } = await request.json().catch(()=>({}));
    if (!/^0(7|1)\d{8}$/.test(String(phone||""))) return json({ ok:false, error:"Invalid phone" }, { status:400 });
    ...
    await q(
      ...
      "VALUES (?, ?, ?, ?, 0, 0, ?, ?)",
      phone, full_name || null, id_number || null, account, pass_hash, nowSec()
    ).run();
  ...
  // ----- public: create tx -----
  if (pathname === "/tx" && request.method === "POST") {
    await ensureSchema();
    const { phone, type, amount, detail = "" } = await request.json().catch(()=>({}));
    ...
    await q(
      ...
      id, phone, String(type), Number(amount), detail, Date.now()
    ).run();
    return json({ ok:true, id, status:"Pending" }, { status:201 });
  }

  // ----- public: list tx by phone -----
  if (pathname === "/tx/list" && request.method === "GET") {
    await ensureSchema();
    const phone = url.searchParams.get("phone") || "";
    if (!phone) return json({ ok:false, error:"phone required" }, { status:400 });
    const res = await q(
      "SELECT id, phone, type, amount, detail, status, ts FROM transactions WHERE phone=? ORDER BY ts DESC LIMIT 500",
      phone
    ).all();
  ...
  if (pathname === "/admin/user" && request.method === "GET") {
    if (!(await needAdmin())) return json({ ok:false, error:"Unauthorized" }, { status:401 });
    await ensureSchema();
    const phone = url.searchParams.get("phone") || "";
  ...
  if (pathname === "/admin/user/upsert" && request.method === "POST") {
    if (!(await needAdmin())) return json({ ok:false, error:"Unauthorized" }, { status:401 });
    await ensureSchema();
  ...
  if (pathname === "/admin/pending" && request.method === "GET") {
    if (!(await needAdmin())) return json({ ok:false, error:"Unauthorized" }, { status:401 });
    await ensureSchema();
    const res = await q(
  ...
  if (pathname === "/admin/tx/update" && request.method === "POST") {
    if (!(await needAdmin())) return json({ ok:false, error:"Unauthorized" }, { status:401 });
    await ensureSchema();
